...
// }
KeyFrame::KeyFrame(const std::string& directory, g2o::HyperGraph* graph) :  odom(Eigen::Isometry3d::Identity()), accum_distance(-1), cloud(nullptr), node(nullptr) {
  load(directory, graph);
}
...
bool KeyFrame::load(const std::string& directory, g2o::HyperGraph* graph) {
  ...
  if(graph->vertices().find(node_id) == graph->vertices().end()) {
    ...
    std::cout << "vertex ID= " << node_id << "  does not exist!!" << std::endl;
    return false;
  }
