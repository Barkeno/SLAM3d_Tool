...
#include <view/window/version_modal.hpp>
#include <view/window/graph_edit_window.hpp>
#include <view/window/plane_detection_window.hpp>
#include <view/window/plane_alignment_modal.hpp>
#include <view/window/manual_loop_close_model.hpp>
#include <view/window/edge_refinement_window.hpp>
#include <view/window/automatic_loop_close_window.hpp>
...
private:
  ...
  void main_menu() {
    ...
    bool open_rawdata_dialog = false;
    if(ImGui::BeginMenu("File")) {
      if(ImGui::BeginMenu("Open")) {
        if(ImGui::MenuItem("New data")) {
          open_rawdata_dialog = true;
        }
    ...
    // open dialogs
    open_raw_data(open_rawdata_dialog);
...
private:
   ...
   * @param open_dialog
   */
  void open_raw_data(bool open_dialog)
  {
    // show the progress modal until loading will be finished
    if(progress->run("dataset load")) {
      auto result = progress->result<std::shared_ptr<InteractiveMappingView>>();
      if(result == nullptr) {
        pfd::message message("Error", "failed to load data", pfd::choice::ok);
        while(!message.ready()) {
          usleep(100);
        }
        return;
    ...
    // File open
    auto dialog = pfd::open_file("Choose dataset file", DEFAULT_PATH,
                            { "bag Files (.bag)", "*.bag",
                              "All Files", "*" },
                            false);
    ...
    std::string result;
    for (auto const &name : dialog.result())
    {
        result = name;
    }
...
private:
  ...
  std::shared_ptr<InteractiveMappingView> mappinggraph;
